...
cargo run -p qmd-cli -- bench --rounds 5

# CPU-only (QMD_FORCE_CPU=1)
QMD_FORCE_CPU=1 cargo run -p qmd-cli -- bench --rounds 5

# ORT CoreML (requires --features ort-backend)
QMD_INFERENCE_BACKEND=ort QMD_ORT_EP=coreml \
  cargo run -p qmd-cli --features ort-backend -- bench --rounds 5
...
cargo run -p qmd-cli -- eval --mode vec --verbose
cargo run -p qmd-cli -- eval --mode hybrid --verbose
```
