//! Store — the main entry point for all qmd-core operations.
//!
//! Orchestrates rusqlite (metadata), Tantivy (BM25), usearch (HNSW), and
//! the InferenceBackend (embed/rerank) to provide the same search pipeline
//! as qmd's TypeScript store.ts.
...
/// Candidate pool size for reranking (mirrors TS RERANK_CANDIDATE_LIMIT).
...
impl Store {
    ...
    pub fn index_document(
        ...
        // 2. Add to Tantivy FTS. filepath = "collection/path" (matches TS convention).
    ...
    /// and commands that never run vector search (e.g. `qmd eval --mode bm25`).
    ...
    pub fn search_vec(
        ...
        for (vid, sim) in raw {
            if let Some((doc, body)) = doc_for_vid(&self.db, vid)? {
                ...
                let chunks = chunk_document(&body);
                let best = chunks.first().map(|c| c.text.clone()).unwrap_or_default();
                let docid = docid_from_hash(&doc.hash).to_string();
                results.push(SearchResult {
                    file: format!("qmd://{}", filepath),
    ...
    /// Mirrors qmd's `hybridQuery()` in store.ts.
    pub fn hybrid_query(
        &mut self,
        query: &str,
        limit: usize,
        ...
        // Phase 1: expansion requires generate model (Phase 4). Skip for now.
        let _ = strong_signal;
        ...
        // Step 5: Resolve candidates to full documents + chunk selection.
        ...
        let chunk_texts: Vec<String> = candidate_docs
            .iter()
            .map(|(_, _, body)| best_chunk_text(body, &query_terms).unwrap_or_default())
            .collect();
        ...
        for (i, (cand, hash, body)) in candidate_docs.into_iter().enumerate() {
            let (best_chunk_text, best_chunk_pos) = best_chunk_with_pos(&body, &query_terms);
...
fn best_chunk_text(body: &str, query_terms: &[String]) -> Option<String> {
    let chunks = chunk_document(body);
    if chunks.is_empty() {
        return None;
    }
    if query_terms.is_empty() {
        return chunks.into_iter().next().map(|c| c.text);
    }
    chunks
        .into_iter()
        .max_by_key(|c| {
...
fn best_chunk_with_pos(body: &str, query_terms: &[String]) -> (String, usize) {
    let chunks = chunk_document(body);
    if chunks.is_empty() {
        return (String::new(), 0);
    }
...
/// Format: "2024-01-15T10:30:00Z" (approximate; no chrono dep in Phase 1).
...
/// Mirrors qmd's getEmbeddingFingerprint() — used to detect stale embeddings.
