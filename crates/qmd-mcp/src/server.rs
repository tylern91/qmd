...
impl QmdServer {
    ...
    fn ml(&self) -> anyhow::Result<std::sync::MutexGuard<'_, Store>> {
        let store = self.ml_store.get_or_try_init(|| {
            eprintln!("[qmd-mcp] Loading inference backend (models download on first run)...");
            let backend = LlamaCppBackend::new(LlamaCppConfig::default())
                .context("failed to init LlamaCpp backend")?;
            eprintln!("[qmd-mcp] Backend ready.");
...
pub struct QueryInput {
    /// Plain-text search query. Auto-expanded into lex/vec/hyde variants, fused
    /// with Reciprocal Rank Fusion, and cross-encoder reranked.
    pub query: String,
...
#[tool_handler]
impl ServerHandler for QmdServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            .with_server_info(Implementation::new("qmd", env!("CARGO_PKG_VERSION")))
            .with_instructions(
...
fn format_results(results: &[qmd_core::SearchResult], query: &str) -> String {
    if results.is_empty() {
