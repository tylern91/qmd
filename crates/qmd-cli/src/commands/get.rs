...
/// Parse a path spec: "collection/path.md", "#docid", or "qmd://collection/path.md".
struct PathSpec {
    collection: String,
    path: String,
}
...
pub fn run_ls(index_dir: &Path, path: Option<&str>) -> Result<()> {
    ...
    let (filter_collection, filter_prefix) = match path {
        None => (None, None),
        Some(p) => {
            let p = p.trim_start_matches("qmd://");
            match p.split_once('/') {
    ...
    if let Some(ref col) = filter_collection {
        ...
        for doc in &docs {
            ...
            println!("qmd://{}/{}", doc.collection, doc.path);
        }
    } else {
