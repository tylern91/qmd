...
#[derive(Parser)]
#[command(name = "qmd", version, about, long_about = None)]
struct Cli {
    /// Override the index directory (default: ~/.cache/qmd-rs/)
    #[arg(long, env = "QMD_INDEX_DIR", global = true)]
    index_dir: Option<String>,

    /// Inference backend: llama (default, GGUF via llama-cpp-2) or ort (ONNX Runtime)
    #[arg(long, env = "QMD_INFERENCE_BACKEND", global = true)]
    backend: Option<String>,

    /// ORT execution provider: auto (default), coreml, cuda, directml, cpu
    #[arg(long, env = "QMD_ORT_EP", global = true)]
...
enum Commands {
    ...
    /// Search quality evaluation against synthetic fixtures (mirrors eval.test.ts)
    Eval {
...
fn main() -> Result<()> {
    ...
    if let Some(b) = &cli.backend {
        std::env::set_var("QMD_INFERENCE_BACKEND", b);
    }
    if let Some(ep) = &cli.ort_ep {
        std::env::set_var("QMD_ORT_EP", ep);
    }
