...
//!   QMD_INFERENCE_BACKEND=llama|ort   (default: llama)
//!   QMD_ORT_EP=auto|coreml|cuda|directml|cpu
//!
...
impl LlamaCppBackend {
    ...
    pub fn new(mut config: LlamaCppConfig) -> Result<Self> {
        // Honour QMD_FORCE_CPU=1: disable Metal/CUDA offload for both models.
        // Matches the TS original's QMD_FORCE_CPU contract documented in README.
        let force_cpu = std::env::var("QMD_FORCE_CPU")
...
impl BackendKind {
    pub fn from_env() -> Self {
        match std::env::var("QMD_INFERENCE_BACKEND")
            .unwrap_or_default()
            .to_ascii_lowercase()
            .as_str()
        {
